        ...
        case Network.NET_MESSAGE_GENERIC_TEXT: {
            ...
            if (Packet::Contains(text, "tankIDName") && Packet::Contains(text, "tankIDPass") && 
                ...
                if (Packet::Contains(text, "action|enter_game") && !Packet::Contains(text, "action|enter_game\ninvitedWorld|")) {
                    ...
                    if (itemID > 0) {
                        auto it = std::find_if(items.begin(), items.end(), [&itemID](const Items& i) { return i.id == itemID; });
                        if (it != items.end()) {
                            if (action == "drop") Player.Drop(itemID, Player.GetItemCount(itemID), it->raw_name, CLIENT, SERVER);
                            else Player.Trash(itemID, Player.GetItemCount(itemID), it->raw_name, CLIENT, SERVER);
                            return false;
                        }
                        else LOG_ERROR("Item ID: {} couldn't find in the database!", itemID);
                    }
                    return true;
                }
            }
