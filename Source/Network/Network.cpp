...
std::string NetworkManager::Setup(int which) {
    if (which == 1) {// Bridge Proxy
        enet_address_set_host(&PROXY_ADDRESS, Local.IP.c_str());
        PROXY_ADDRESS.port = Local.UDP;

        PROXY_HOST = enet_host_create(&PROXY_ADDRESS, 1024, 2, 0, 0);
    ...
    if (which == 2) {// Growtopia Server
        SERVER_HOST = enet_host_create(NULL, 1024, 2, 0, 0);
        if (!SERVER_HOST) return "Couldn't create the SERVER_HOST!";

        enet_address_set_host(&SERVER_ADDRESS, Server.IP.c_str());
        SERVER_ADDRESS.port = Server.UDP;

        SERVER_HOST->usingNewPacket = 1;
...
void NetworkManager::Injector() {
    ...
    if (enet_initialize() != 0) LOG_ERROR("Couldn't initialize the enet!");
    ...
    while (Http.server_data_cache.empty()) {
    ...
    while (STATUS == 2) {
        while (PROXY_HOST && enet_host_service(PROXY_HOST, &PROXY_EVENT, 1) > 0) {
            ...
            switch (PROXY_EVENT.type) {
                case ENET_EVENT_TYPE_CONNECT: {
                    ...
                    LOG_WARN("Waiting for the Proxy to connect the Server");
        ...
        while (SERVER_HOST && enet_host_service(SERVER_HOST, &SERVER_EVENT, 1) > 0) {
