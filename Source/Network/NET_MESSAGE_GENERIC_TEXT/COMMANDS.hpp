...
public:
    static inline bool Inject(std::string command, ENetPeer* CLIENT, ENetPeer* SERVER) {
        ...
        if (command == "/proxy") {
			...
			p.text += "add_custom_textbox|`4/farm  `o(Opens Auto Farm Settings Menu)|size:small|\n";
			p.text += "add_custom_textbox|`4/spam  `o(Opens Auto Spam Settings Menu)|size:small|\n";
			p.text += "add_custom_textbox|`4/wrench  `o(Opens/Closes Wrench Mode (Left = Pull & Right = Kick)|size:small|\n";
			p.text += "add_spacer|small|\n";
			...
			std::thread([NEEDS, myBGl, myDL, myWL, CLIENT, SERVER]() mutable {
				...
				if (dropBGL > 0) {
					...
					Player.Drop(7188, dropBGL, "Blue Gem Lock", CLIENT, SERVER);
					myBGl -= dropBGL;
				...
				if (dropDL > 0) {
					...
					Player.Drop(1796, sendDL, "Diamond Lock", CLIENT, SERVER);
					myDL -= sendDL;
					std::this_thread::sleep_for(std::chrono::milliseconds(120));
				}
				...
				if (dropWL > 0) {
					...
					Player.Drop(242, sendWL, "World Lock", CLIENT, SERVER);
					myWL -= sendWL;
				}
			}).detach();
            ...
            std::thread([CLIENT, SERVER]() {
                if (Player.HasItem(7188)) {
                    Player.Drop(7188, Player.GetItemCount(7188), "Blue Gem Lock", CLIENT, SERVER);
                    std::this_thread::sleep_for(std::chrono::milliseconds(120));
                }

                if (Player.HasItem(1796)) {
                    Player.Drop(1796, Player.GetItemCount(1796), "Diamond Lock", CLIENT, SERVER);
                    std::this_thread::sleep_for(std::chrono::milliseconds(120));
                }

                if (Player.HasItem(242)) {
                    Player.Drop(242, Player.GetItemCount(242), "World Lock", CLIENT, SERVER);
                }
            }).detach();
