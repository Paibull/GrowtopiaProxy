...
inline void Serialize(GamePacketBuilder& p, const GamePacket<OnConsoleMessage>& v) { p.Insert("`o[`4GrowtopiaProxy`o]`w`w`w " + v.text); }
inline void Serialize(GamePacketBuilder& p, const GamePacket<OnTalkBubble>& v) { p.Insert(v.userId); p.Insert(v.text); if (v.unknown != 31) p.Insert(v.unknown); if (v.unknown2 != 31) p.Insert(v.unknown2); }
