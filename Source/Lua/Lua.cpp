...
LuaManager::LuaManager() {
    ...
    m_scriptsDir = (projectRoot / "Scripts").string();

    LOG_ERROR("{}", m_scriptsDir);
...
void LuaManager::Inject(const std::string& filename) {
    ...
    std::thread([this, filename]() {
        std::filesystem::path scriptPath = std::filesystem::path(m_scriptsDir) / filename;
        if (!std::filesystem::exists(scriptPath)) {
            LOG_ERROR("Couldn't find the {}", scriptPath.string());
            m_running = false;
            m_mutex.unlock();
            return;
        }
        ...
        if (!result.valid()) {
            ...
            LOG_DEBUG("{} injected successfully", filename);
        }
